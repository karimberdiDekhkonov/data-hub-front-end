...
function Month() {
  ...
  const setActiveMonthsToLocal = (monthName) =>{
    setActive(monthName); 
  ...
  return (
    <Dropdown>
      <Dropdown.Toggle className="fw-bold w-100 text-start d-flex gap-3" variant="light" id="dropdown-basic">
        <svg xmlns="http://www.w3.org/2000/svg" width="22" height="22" fill="currentColor" class="bi bi-calendar2-month" viewBox="0 0 16 16">
      ...
      <Dropdown.Menu>
          <div className="months d-flex flex-column gap-2 justify-content-center">
            {
              activeMonths.map((monthId) =>{
                return <Button href='/owner/tableArea' onClick={()=>setActiveMonthsToLocal(months[monthId])} variant={`outline-primary`}>{months[monthId]}</Button>
              })
            }
          </div>  
      </Dropdown.Menu>
