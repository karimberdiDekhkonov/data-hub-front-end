...
function Example() {
  ...
  const editName = async() => {
    if(name===null || name.length > 15){
      alert("Name space is empty or name length is so high !");
      handleClose();
      return;
    }
  ...
  return (
    <>
      ...
      <Modal show={show} onHide={handleClose}>
        ...
        <Modal.Body>
          <input onChange={(e)=>{setName(e.target.value)}} className='form-control' placeholder='Enter your new company name' type="text" name="" id="" />
        </Modal.Body>
        <Modal.Footer>
